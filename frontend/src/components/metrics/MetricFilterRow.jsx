...
/**
 * Single filter row inside the metric builder. Mirrors SegmentRuleRow
 * (same 9 operators) but uses the `customMetrics.*` i18n keys and the
 * `customMetrics.filterRemove` aria-label. The shared operator
 * vocabulary lets a filter from a custom metric be promoted to a
 * segment rule without translation.
 *
 * Pure presentational — `onChange(rule)` / `onRemove()` are wired by
 * the parent `MetricBuilderPage`.
 */
export default function MetricFilterRow({ rule, onChange, onRemove }) {
    ...
    <div className="flex flex-wrap items-end gap-2 rounded border p-2">
      <div className="min-w-[140px] flex-1 space-y-1">
        ...
        <Input
          ...
          placeholder="country, plan, amount, …"
        />
      </div>
      ...
      <Button
        ...
        size="sm"
        onClick={onRemove}
        aria-label={t('customMetrics.filterRemove')}
      >
